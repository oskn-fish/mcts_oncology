...
class PPODefaultCallbacks(DefaultCallbacks):
        ...
        if hasattr(env, "DLT_max"):
            if episode.length == 1:
                ...
                if "penalty_metric" in episode._last_infos["agent0"].keys():
                    episode.custom_metrics["penalty_metric"] = [
                        episode._last_infos["agent0"]["penalty_metric"]
                    ]
                if "penalty" in episode._last_infos["agent0"].keys():
                    episode.custom_metrics["penalty"] = [
                        episode._last_infos["agent0"]["penalty"]
                    ]
                episode.custom_metrics["MTD_metric"] = [
                    episode._last_infos["agent0"]["MTD_metric"]
                ]
                episode.custom_metrics["DLTs"] = [
                ...
                # )
                if "penalty_metric" in episode._last_infos["agent0"].keys():
                    episode.custom_metrics["penalty_metric"].append(
                    episode._last_infos["agent0"]["penalty_metric"]
                    )
                if "MTD_metric" in episode._last_infos["agent0"].keys():
                    episode.custom_metrics["MTD_metric"].append(
                    episode._last_infos["agent0"]["MTD_metric"]
                )
                if "penalty" in episode._last_infos["agent0"].keys():
                    episode.custom_metrics["penalty"].append(
                    episode._last_infos["agent0"]["penalty"]
                    )
                episode.custom_metrics["DLTs"].append(
                    episode._last_infos["agent0"]["DLTs"]
                )
    ...
    def on_postprocess_trajectory(
        ...
        **kwargs,
        ...
        if "penalty_metric" in episode.custom_metrics.keys():
            postprocessed_batch["penalty_metric"] = np.array(
            episode.custom_metrics["penalty_metric"]
        )
        if "penalty" in episode.custom_metrics.keys():
            postprocessed_batch["penalty"] = np.array(
            episode.custom_metrics["penalty"]
            )
        if "MTD_metric" in episode.custom_metrics.keys():
            postprocessed_batch["MTD_metric"] = np.array(
                episode.custom_metrics["MTD_metric"]
            )
        postprocessed_batch["DLTs"] = np.array(
            episode.custom_metrics["DLTs"]
        )
...
class PPOConfig(PGConfig):
    ...
    def __init__(self, algo_class=None):
        ...
        self.callbacks_class = PPODefaultCallbacks
...
class PPO(Algorithm):
    ...
    def training_step(self) -> ResultDict:
        ...
        if "penalty_metric" in  train_batch.policy_batches["default_policy"].keys():
            train_results["default_policy"]["learner_stats"]["penalty_metric"] = np.sum(
                train_batch.policy_batches["default_policy"]["penalty_metric"]
            ) / np.count_nonzero(
                train_batch.policy_batches["default_policy"]["penalty_metric"]
            )
        if "penalty" in  train_batch.policy_batches["default_policy"].keys():
            train_results["default_policy"]["learner_stats"]["penalty"] = np.sum(
                train_batch.policy_batches["default_policy"]["penalty"]
            ) / np.count_nonzero(
                train_batch.policy_batches["default_policy"]["penalty"]
            )
        train_results["default_policy"]["learner_stats"]["MTD_metric"] = np.sum(
