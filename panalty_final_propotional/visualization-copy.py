...
"""
(penalty_coefficient_abs=0.0120 | ....) , (scenario_0 | ...), (['DLTs', 'penalty', 'MTD_metric', 'rewards'])
"""

# %%

# add penalties to reward
penalties = sorted([penalty/10 for penalty in [0.2834043767146469, 0.2203114013575012, 0.18340437671464696, 0.15721842600035546, 0.13690702464285426, 0.12031140135750121, 0.10628000179850475, 0.09412545064320973, 0.08340437671464696, 0.0738140492857085]])

def add_penalty_to_reward(dic):
    for penalty, penalty_coeff in zip(penalties, sorted(dic.keys())):
        # print(penalty)
        # print(penalty_coeff)
        for scenario in dic[penalty_coeff].values():
            scenario["rewards_with_penalty"] = np.array(scenario["rewards"])-penalty*np.array(scenario["DLTs"])
add_penalty_to_reward(all_agents_20_scenarios_ppo)
add_penalty_to_reward(all_agents_10_scenarios_ppo)
add_penalty_to_reward(all_agents_20_scenarios_alphazero)
add_penalty_to_reward(all_agents_10_scenarios_alphazero)
...
merged_all_agents_20_scenarios_ppo = merge_scenarios(all_agents_20_scenarios_ppo)
merged_all_agents_10_scenarios_ppo = merge_scenarios(all_agents_10_scenarios_ppo)
merged_all_agents_20_scenarios_alphazero = merge_scenarios(all_agents_20_scenarios_alphazero)
merged_all_agents_10_scenarios_alphazero = merge_scenarios(all_agents_10_scenarios_alphazero)

"""
(penalty_coefficient_abs=0.0120 | ....) , (['DLTs', 'penalty', 'MTD_metric', 'rewards'])
"""

#%%
fig, ax = plt.subplots()
ax.hist(merged_all_agents_20_scenarios_ppo["penalty_coefficient_abs=0.0120"]["rewards"])
...
fig, ax = plt.subplots()
DLTs = ([np.mean(merge_scenarios(all_agents_20_scenarios_ppo[key_agent])["DLTs"]) for key_agent in sorted(all_agents_20_scenarios_ppo.keys())])
MTD_metric = ([(np.mean(merge_scenarios(all_agents_20_scenarios_ppo[key_agent])["MTD_metric"])/2+1/2)*100 for key_agent in sorted(all_agents_20_scenarios_ppo.keys())])


a, b = np.polyfit(DLTs, MTD_metric, 1)
y = a*np.linspace(4, 10,len(MTD_metric))+b

DLTs_ = ([np.mean(merge_scenarios(all_agents_20_scenarios_alphazero[key_agent])["DLTs"]) for key_agent in sorted(all_agents_20_scenarios_alphazero.keys())])
MTD_metric_ = ([(np.mean(merge_scenarios(all_agents_20_scenarios_alphazero[key_agent])["MTD_metric"])/2+1/2)*100 for key_agent in sorted(all_agents_20_scenarios_alphazero.keys())])

a_, b_ = np.polyfit(DLTs_, MTD_metric_, 1)
y_ = a_*np.linspace(4, 10,len(DLTs))+b_

lines = [[[penalty_ppo, reward_ppo], [penalty_alphazero, reward_alphazero]] for penalty_ppo, reward_ppo, penalty_alphazero, reward_alphazero in zip(DLTs, MTD_metric, DLTs_, MTD_metric_)]
lc =  LineCollection(lines, colors=colors, linewidth=2)

ax.plot(np.linspace(4, 10,len(DLTs)), y, color='lightcoral')
ax.plot(np.linspace(4, 10,len(DLTs)), y_, color='cornflowerblue')
ax.add_collection(lc)
ax.scatter(DLTs, MTD_metric, c='red', label="RLE")
ax.scatter(DLTs_, MTD_metric_, c='blue', label="提案手法")

fontsize=20
ax.set_xlabel("DLT発現人数", fontsize=fontsize)
