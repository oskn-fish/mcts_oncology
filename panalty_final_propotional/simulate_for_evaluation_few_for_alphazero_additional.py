...
def remote_sample_simulations(agent_path, env_func, env_config, num_simulations):
    ...
    metrics = {"DLTs":[], "penalty_metric":[], "MTD_metric":[], "rewards":[]}
    ...
    for _ in range(num_simulations):
        ...
        metrics["penalty_metric"].append(info["penalty_metric"])
...
def evaluate_on_20_senarios(agent_path):
    ...
    for scenario in tqdm(range(20), leave=False):
        env_config = {"D": 6, "N_cohort": 3, "N_total": 36, "phi": 0.25, "scenario": scenario, "DLT_max": 10, "penalty_abs": 0}
        result = sample_env_simulations(agent_path, RLEEnv_expectation_negative_reward_penalty_continue, env_config)
...
def evaluate_on_10_senarios(agent_path):
    ...
    for scenario in tqdm(range(10), leave=False):
        ...
        result = sample_env_simulations(agent_path, RLEEnv_expectation_negative_reward_penalty_continue_evaluate, env_config)
        results["scenario_"+str(scenario)] = result
    return results    


# checkpointのフォルダ名からpenalty_abs部分を抽出して，キーに入れる
def sample(list_checkpoint_path):
    pattern = ".*(penalty_abs=0\.?\d*).*"
    ...
    # pickleで保存
    start_index = 0
    while os.path.isfile(os.path.join(os.path.dirname(__file__),"all_agents_all_scenarios_alphazero_few_"+str(start_index)+".pickle")):
        start_index += 1
    FILE_NAME = "all_agents_all_scenarios_alphazero_few_"+str(start_index)+".pickle"
...
# path_checkpoint_root_ppo = "/home/chinen/esc_mcts/panalty_final/PPO_2024-01-19_19-07-16/"
...
path_checkpoint_root_alphazero = "/home/chinen/esc_mcts/panalty_final/AlphaZero_2024-01-19_20-34-15/"
...
while True:
    sample(list_checkpoint_path_alphazero)
# FILE_NAME_FEW = 'all_agents_all_scenarios_alphazero_few.pickle'
# if not os.path.isfile(os.path.join(os.path.dirname(__file__),FILE_NAME_FEW)):
#     sample(list_checkpoint_path_alphazero, FILE_NAME_FEW)