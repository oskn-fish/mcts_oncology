...
class RLEEnv_negative_reward(gym.Env):
    ...
    def step(self, action):
        ...
        if action in [0, 1, 2]:  # down/stay/up
            ...
            if type(self.posteriors) == np.ndarray:
                assert self.MTD_true == None and self.p_true == None
                reward = self.sample_reward(self.last_obs, MTD)  # 投与せず，last_obsの交信もない
            else:
                # reward = 1 if self.MTD_true == MTD else 0
                reward = 1 if self.MTD_true == MTD else -1
            truncated = False
    ...
    def sample_reward(self, obs, MTD):
        posterior = self.cal_posterior(obs)
        sampled_index = np.random.choice(len(posterior), p=posterior)
        reward = 1 if self.MTD_trues[sampled_index] == MTD else -1
        return reward
