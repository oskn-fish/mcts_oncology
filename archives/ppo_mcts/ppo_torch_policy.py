...
import ray
from ppo_mcts.ppo_tf_policy import validate_config
...
class PPOTorchPolicy(
    ...
    def __init__(self, observation_space, action_space, config):
        config = dict(ppo_mcts.ppo.PPOConfig().to_dict(), **config)
    ...
    def loss(
        ...
        if self.config["mcts_on"]:
            priors = nn.Softmax(dim=-1)(logits)
            cross_entropy_loss = torch.mean(
                # -torch.sum(
                #     train_batch["mcts_policies"]
                #     # * torch.log(curr_action_dist + self.config["epsilon"]),
                #     * torch.log(priors),
                #     dim=-1,
                # )
                # """one hot のときのみ使える．
                # 1が立っていないところのlogを計算しないようにする
                # """
                -torch.sum(
                    torch.log(1 - train_batch["mcts_policies"] * (1 - priors - 1e-6))
                )
            )
    ...
    def postprocess_trajectory(
        ...
        if episode == None:
            sample_batch["mcts_policies"] = np.array(
                [[0] * self.action_space.n] * len(sample_batch["t"])
            )[sample_batch["t"].astype(int)]
        else:
            sample_batch["mcts_policies"] = np.array(
