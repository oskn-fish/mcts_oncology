...
class MCTS:
    ...
    def compute_action(self, node: Node):
        for _ in range(self.num_sims):
            leaf = node.select()
            if leaf.done:
                value = leaf.reward
            else:
                # NOTE: leafまできているので，あとは play out
                child_priors, value = self.model.compute_priors_and_value(leaf.obs) # NOTE: prior = 行動選択確率 = 方策 π
                if self.add_dirichlet_noise:
                    child_priors = (1 - self.dir_epsilon) * child_priors # NOTE: 探索を促進するために，ディリクレのノイズをroot nodeのpriorに添加．（optional. 論文記載あり）
