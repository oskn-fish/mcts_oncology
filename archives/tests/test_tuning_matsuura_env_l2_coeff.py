...
config = AlphaZeroConfig().training(ranked_rewards={"enable": False}, model={"custom_model":DenseModelWithPrediction}, l2_coeff=tune.grid_search([0.0, 0.001, 0.0001, 0.00001])).resources(num_gpus=1).rollouts(num_rollout_workers=15).environment(env=RLEEnv_modified, env_config=env_config, disable_env_checking=True).offline_data(output="./offline_data").training(mcts_config = mcts_config)
...
tuner = tune.Tuner(
    ...
    run_config = air.RunConfig(
        callbacks = [WandbLoggerCallback(project="no_early_stop_with_prediction_on_matsuura_l2coeff", api_key_file=os.path.join(os.path.dirname(__file__), "wandb_api_key.txt"))],
