...
config = AlphaZeroConfig().training(ranked_rewards={"enable": False}, model={"custom_model":DenseModelWithPrediction}, l2_coeff=0.001, replay_buffer_config=replay_buffer_config, sgd_minibatch_size=tune.grid_search([32, 64, 128, 256])).resources(num_gpus=1).rollouts(num_rollout_workers=6).environment(env=RLEEnv_modified, env_config=env_config, disable_env_checking=True).offline_data(output="./offline_data").training(mcts_config = mcts_config)
...
tuner = tune.Tuner(
    ...
    run_config = air.RunConfig(
        callbacks = [WandbLoggerCallback(project="no_early_stop_with_prediction_on_matsuura_minibatch", api_key_file=os.path.join(os.path.dirname(__file__), "wandb_api_key.txt"))],
