...
env_config = {"D":6, "N_cohort":3, "N_total":36}
...
replay_buffer_config = {
            ...
            "capacity": tune.grid_search([4096, 10240]),
            # Choosing `fragments` here makes it so that the buffer stores entire
            # batches, instead of sequences, episodes or timesteps.
            "storage_unit": "fragments",
        }

num_steps_sampled_before_learning_starts = tune.grid_search([1024, 4096, 10240])
lr = tune.grid_search([1e-3, 1e-4, 1e-5])
train_batch_size = tune.grid_search([1024, 2048, 4096])
epsilon = 1e-12
l2_coeff = tune.grid_search([1e-3, 1e-4, 1e-5])

config = AlphaZeroConfig().training(ranked_rewards={"enable": False}, model={"custom_model":DenseModel}, l2_coeff=l2_coeff, replay_buffer_config=replay_buffer_config, num_steps_sampled_before_learning_starts=num_steps_sampled_before_learning_starts, lr=lr,train_batch_size=train_batch_size,epsilon=epsilon).resources(num_gpus=1).rollouts(num_rollout_workers=6).environment(env=RLEEnv_expectation, env_config=env_config, disable_env_checking=True).offline_data(output="./offline_data").training(mcts_config = mcts_config)
...
tuner = tune.Tuner(
    ...
    run_config = air.RunConfig(
        callbacks = [WandbLoggerCallback(project="matsuura_env_with_posterior", api_key_file=os.path.join(os.path.dirname(__file__), "wandb_api_key.txt"))],
